...
| Loki (OpenShift Logging) | Container logs per namespace, one day at a time for offline analysis                         | OpenShift console, Observe → Logs; or `scripts/loki-dump.sh prod 2026-09-23` for an NDJSON dump | OpenShift login, same group as the namespace (`oc login` first)                                                                                                                                                   |
| OpenShift console, `oc`  | Pods, logs, ExternalSecret status, rollouts                                                  | One console per cluster, links in Operations                                                    | Granted by a namespace admin (DRP roster). The prod admin binding is `admin-SVC1751` (ClusterRole `admin`), tied to the SVC1751 service unit; which EPFL group feeds it is being confirmed. Then `oc login --web` |
| ArgoCD                   | Sync state, diff, rollback                                                                   | One per cluster, links in Operations                                                            | OpenShift login                                                                                                                                                                                                   |
| GlitchTip                | Frontend JavaScript errors, with a `trace_id` expected to match Tempo (not yet verified)     | [enac-it-glitchtip.epfl.ch](https://enac-it-glitchtip.epfl.ch)                                  | Dedicated account, not SSO. Team credentials are in Infisical under `ENAC-IT K8S/k8s-prod/epfl-enac/glitchtip`. Details: [Frontend Error Monitoring](../frontend/error-monitoring.md)                             |
...
OpenShift Pipelines and system roles. Re-check before trusting this line:

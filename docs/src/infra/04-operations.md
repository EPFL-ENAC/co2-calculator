...
Every alert in `openshift-app-config` carries a `runbook_url` that lands
here, or on
[which knob, when an alert fires](../database/02-connection-budget.md#which-knob-when-an-alert-fires)
for the DB rules. Keep both headings as they are, or update the three
...
| `DbPoolSaturationHigh`, then `DbPoolCheckoutTimeout`      | DB layer 1: this pod's pool (`DB_POOL_SIZE` + `DB_MAX_OVERFLOW`) is full. Behind the bouncer a backend pod should have scaled out; on the worker, jobs are queuing on connections                                                                                                                                                                                                                                                                                                                 | Check the HPA, then the Grafana DB pool panel; [which knob](../database/02-connection-budget.md#which-knob-when-an-alert-fires); read plans [2566](../implementation-plans/2566-db-pool-disposal-and-visibility.md) and [1723](../implementation-plans/1723-job-concurrency-and-db-pool.md) before touching pool sizes |
| `DbBouncerQueued`, then `DbBouncerQueueTimeout`           | DB layer 2: more than 60 (dev) or 70 (stage, prod) transactions in flight at once filled PgBouncer's server pool (transaction pooling on all three since 2026-09-24). Reference event 2026-09-17 17:07 to 17:13, under the old 25-slot session pool and 120 s wait: total open crossed 25, fresh logins queued 120 s while holding pod pool slots, backend pools hit 100%, 5 s pool timeouts, `/ready` red, a 503 band. At today's 10 s wait the same overrun is a short error band, not a lockup | Nothing local heals it. Stray client first (`application_name` in `pg_stat_activity`), then a long transaction holding slots (COPY factor upsert, long routes), then a bigger `default_pool_size` from DBaaS; [which knob](../database/02-connection-budget.md#which-knob-when-an-alert-fires)                         |
| `DbServerConnectionSlotsExhausted`                        | DB layer 3: Postgres itself is full (53300). Orphaned backends from dead pods, or a laptop on the shared DB. The 2026-08-31 mode: everyone is locked out, pgAdmin and migrations included                                                                                                                                                                                                                                                                                                         | Count orphans: `select state, count(*), max(now()-state_change) from pg_stat_activity where usename='app' group by state`; `pg_terminate_backend` the idle ones; `SI_POSTGRESQL` if `max_connections` must grow; [which knob](../database/02-connection-budget.md#which-knob-when-an-alert-fires)                      |

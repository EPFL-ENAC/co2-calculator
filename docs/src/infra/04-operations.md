...
| `DbBouncerQueued`, then `DbBouncerQueueTimeout` (dev only) | DB layer 2: the fleet ceiling overran PgBouncer's 25-slot server pool. Reference event 2026-09-17 17:07 to 17:13: total open crossed 25, fresh logins queued 120 s while holding pod pool slots, backend pools hit 100%, 5 s pool timeouts, `/ready` red, a 503 band | Nothing local heals it. Stray client first (`application_name` in `pg_stat_activity`), then fewer pods or smaller ceilings, or a bigger `default_pool_size` from DBaaS; [which knob](../database/02-connection-budget.md#which-knob-when-an-alert-fires)                                          |

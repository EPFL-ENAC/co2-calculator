...
- **Rollout, all under #1556** (decided 2026-07-15):
  1. Backend: extend `/carbon-reports/{id}/modules/{type}` with entry
     CRUD/stats (legacy routes delegate to the shared impl) + planner
     lifecycle endpoints.
  2. Frontend: planner on identity routes; `buildModulePath` swap for
     Calculator/Explore.
  3. Backend: delete legacy unit/year module routes and the
     `carbon_project_type` param.
...
### Slice 1 — Travel + Headcount end-to-end

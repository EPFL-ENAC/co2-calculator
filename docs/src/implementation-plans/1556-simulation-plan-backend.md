...
- The `% of reference year` slider **already exists**:
  ...
  `data.percentage_of_reference_year` (renamed from `percentage_of_last_year`
  2026-07-16), matching the reference-year entry by stable identifiers.
...
Each Calculator module's `schemas.py` splits into (delivered — all 8
modules, no `schemas.py` remains):
...
- `emissions.py` — emission-computation helpers (grew out of the split;
  not in the original three-file spec)
...
- Type-2 modules (Process Emissions, Buildings, Equipments, Research
  ...
  snapshot prefill + `percentage_of_reference_year`.
...
- **Snapshot prefill** (slice 2):
  `POST /carbon-reports/{carbon_report_id}/modules/{module_type}/prefill`
  copies reference-year Calculator entries into the plan module with
  `percentage_of_reference_year = 100`. Note: the slider override computes
...
- [x] Split `modules/<name>/schemas.py` → `data_entries.py` / `factors.py` /
      `handlers.py` / `emissions.py` (all 8 modules, mechanical).
...
- [x] Snapshot-prefill endpoint + service: `POST /carbon-reports/{id}/modules/{module}/prefill` copies the
      reference-year Calculator entries at `percentage_of_reference_year =
...
- [x] Snapshot rows expose `reference_kg_co2eq` (the source entry's summed
      emissions) on the planner submodule items, and the data-entry
      response surfaces `percentage_of_reference_year` — the shared
      table's reference column + slider read both (2026-07-16).

### Slice 3 — Remaining modules + results

- [x] Buildings, Equipments, Research Facilities, External Clouds & AI
      (type-2): delivered 2026-07-16 — all five type-2 modules are
      `behavior: 'prefilled'` in `constant/planner-module-config`; the
      shared table renders the reference-kg column + 0–200% slider in
      planner context, and setting a section's reference year
      auto-prefills every prefilled module.

...
| **S6**     | Frontend config parity: required flags, min/max bounds, drop the 0.001 minimums (or confirm them), integer check for `int` fields, trim before required. Mechanical edits to `module-config/*.ts` + the shared validator only.                                                                                                                  | F-8, F-9, F-13            | ✅ merged in #2231 (v1.4.0) — F-9 0.001-min, int check and F-13 still open, need a product answer |

...
| sub_class                           | optional, class/subclass tuple in factors | optional; an unknown value is a row error (#2591), a missing one is not (`require_subkind_for_factor = False`, equipment only) | **required: true** + stray `min: 0` on a select (`equipment.ts:75-76`)                                                                                                                                  | **DIV** — FE stricter, confirm (N-3) |

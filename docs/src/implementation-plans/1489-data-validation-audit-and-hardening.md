...
| **S4**     | Backend DTO consistency fixes: animal-RF validators (use ≥ 0, non-empty ids), whitespace-strip on required strings everywhere, digits-only `user_institutional_id` (if confirmed).                                                                                                                                                              | F-7 (DTO part), F-9, F-10 | **PR #2231**                                                                        |
...
| **S6**     | Frontend config parity: required flags, min/max bounds, drop the 0.001 minimums (or confirm them), integer check for `int` fields, trim before required. Mechanical edits to `module-config/*.ts` + the shared validator only.                                                                                                                  | F-8, F-9, F-13            | **PR #2231** (F-9 0.001-min, int check and F-13 still open — need a product answer) |

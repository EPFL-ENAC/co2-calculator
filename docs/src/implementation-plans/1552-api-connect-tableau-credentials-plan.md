...
- **Encryption key is dedicated and fails closed.** Set both
  ...
  unset, encrypt/decrypt raise. Never reuse `JWT_HMAC_KEY`.
...
- [x] **Step 2: Add settings** after `TABLEAU_MAX_FIELDS` in `config.py`:
...
- [x] **Step 3: Mirror the new vars in `backend/.env.example`** (near
...
- [x] **Step 4: Mirror them in `helm/templates/backend-deployment.yaml`** —
...
- [x] **Step 5: Add a startup validation** so a misconfigured prod fails at
      boot, not on first save. Call it at the top of `lifespan` in
      `backend/app/main.py` (line ~28). The settings expose no `ENV` string —
      the only environment discriminator is `LOCAL_ENVIRONMENT: bool`
      (`config.py:32`); gate on it:
...
Integration tests boot the real app via `TestClient(app)`; make sure the test
env runs with `LOCAL_ENVIRONMENT=true` (set it in `backend/tests/conftest.py`
if it is not already) so the check never trips the suite.

- [x] **Step 6: Verify it imports**
...
- Modify: `backend/app/core/crypto.py` (exists as a docstring-only stub —
  replace its body)
...
Run: `cd backend && uv run pytest tests/unit/core/test_crypto.py -v`
Expected: FAIL — `ModuleNotFoundError: app.core.crypto`.

- [x] **Step 3: Implement `crypto.py`**
...
git add backend/app/core/crypto.py backend/tests/unit/core/test_crypto.py
...
- Test: `backend/tests/unit/core/test_url_safety.py`
...
Run: `cd backend && uv run pytest tests/unit/core/test_url_safety.py -v`
Expected: FAIL — `ModuleNotFoundError: app.core.url_safety`.

- [x] **Step 3: Implement `url_safety.py`**
...
```bash
cd backend && make type-check && make lint
git add backend/app/core/url_safety.py backend/tests/unit/core/test_url_safety.py
...
- [x] **Step 1: Write the models** (mirror `models/user.py` conventions)
...
- [x] **Step 2: Write the hardcoded registry** (`connectors.py`):
...
- [x] **Step 3: Register models for metadata discovery.** Add to
...
- [x] **Step 2: Review the generated file.** Confirm it creates both tables,
...
Postgres treats each NULL as distinct, so a plain column index would not
catch duplicates when `data_entry_type_id` is NULL (the common case); the
COALESCE expression closes that gap. Mirror it in `downgrade()` and prune any
false-positive `drop_index` lines on unrelated tables (see memory: Alembic
autogenerate quirk).
...
- Test: `backend/tests/unit/repositories/test_connector_repo.py`
...
- [x] **Step 1: Write the failing test** (use the real async `db_session`
      fixture from `backend/tests/conftest.py:51` — note
      `tests/unit/repositories/test_unit_repo.py` mocks its session instead;
...
Run: `cd backend && uv run pytest tests/unit/repositories/test_connector_repo.py -v`
Expected: FAIL — module not found.

- [x] **Step 3: Implement the repos** (mirror `unit_repo.py`):
...
```bash
cd backend && make type-check && make lint
git add backend/app/repositories/connector_repo.py backend/tests/unit/repositories/test_connector_repo.py
...
- Produces: `ConnectorSpecRead`, `ConnectorConnectionCreate` (also doubles as
  the update payload — `secret_value` optional, blank keeps stored),
  `ConnectorConnectionRead` (no secret; `has_secret: bool`),
  `ConnectorDatasourceCreate`, `ConnectorDatasourceRead`.

- [x] **Step 1: Write the schemas** (mirror `schemas/user.py`):
...
- Test: `backend/tests/unit/services/test_connector_service.py`
...
Run: `cd backend && uv run pytest tests/unit/services/test_connector_service.py -v`
Expected: FAIL — module not found.
...
- [x] **Step 4: Add an SSRF-rejection test** to the same file — a
...
- [x] **Step 5: Audit connection mutations.** In `upsert_connection` /
...
```bash
cd backend && make type-check && make lint
git add backend/app/services/connector_service.py backend/tests/unit/services/test_connector_service.py
...
- Test: `backend/tests/integration/v1/test_connectors.py`

**Interfaces:**

- Produces these routes. Gate: `/sync/dispatch` checks permissions inline in
  its body, not via a dependency — reuse instead the
  `Depends(require_permission("backoffice.configuration", "edit"))` pattern
  from `sync_module_factors` (`api/v1/data_sync.py:1005`), the same permission
  the dispatch path requires for global syncs. Deny by default:
...
- [x] **Step 1: Write the failing test.** There is no shared authed-client
      fixture; follow `tests/integration/v1/test_auth.py` /
      `test_permission_scope_e2e.py`: a local sync `TestClient(app)` fixture,
      auth faked by monkeypatching `decode_jwt` + `UserService.get_by_id`, DB
      via `app.dependency_overrides` on the session dep, overrides cleared
      after each test. Adapt the snippet below (written async) to that sync
      idiom:
...
Run: `cd backend && uv run pytest tests/integration/v1/test_connectors.py -v`
...
from app.api.deps import get_db  # same session dep the data_sync routes use (verify name)
from app.models.connector import ConnectorType
from app.schemas.connector import (
    ConnectorConnectionCreate,
    ConnectorConnectionRead,
...
# Deny by default: same gate as sync_module_factors (data_sync.py:1005) —
# require_permission("backoffice.configuration", "edit").
from app.api.deps import require_permission  # verify actual import path in data_sync.py

router = APIRouter(
    tags=["connectors"],
    dependencies=[Depends(require_permission("backoffice.configuration", "edit"))],
)
...
> **Security (A06, A10).** `POST …/test` triggers an outbound call. No rate
> limiter exists in this app (verified — no slowapi/Limiter anywhere): add a
> minimal in-process per-user cooldown (module-level `{user_id: last_ts}`
> map, return 429 inside the window) — do not add slowapi. The `detail` it
> returns must be a
> generic string ("authentication failed" / "host unreachable") — never a raw
> exception, stack trace or Tableau response body.
>
> **Shipped:** the endpoint ships with an in-process, per-user 30-second
> cooldown (429 outside the window) and returns generic details only.

- [x] **Step 4: Register the router** in `backend/app/api/router.py` (same
      place `files.router` is included):
...
- [x] **Step 6: Type-check, lint, commit**

```bash
cd backend && make type-check && make lint
git add backend/app/api/v1/connectors.py backend/tests/integration/v1/test_connectors.py <aggregator>
...
- Produces a base class with:
  ...
  - abstract (shipped): `transform_data` and `_load_data` (inherited from
    `DataIngestionProvider`) — both subclasses implement these, not
    `_build_data_entry`. `_build_data_entry` is a private helper local to
    `HeadcountMembersApiProvider` (Task 5.1), not part of the base contract.

- [x] **Step 1: Create the base class.** Move, verbatim, these methods from
      `professional_travel_api_provider.py` into the base (cut from the named
      ranges, do not rewrite):
  - module-level functions (not methods): `normalize_vds_payload` (82-103),
    `to_bool` (932-933)
  - `_generate_jwt` (563-602), `_signin_with_jwt` (603-678),
    `_create_session` (679-692), `_vds_read_metadata` (693-710),
    `_extract_field_captions` (711-741), `_build_payload` (742-758),
    `_vds_query_datasource` (759-773), `_record_row_error` (917-929,
    `@staticmethod`), `_resolve_carbon_report_modules` (791-915).
...
- [x] **Step 2: Add the classmethod connection test**
...
- [x] **Step 3: Write a unit test** that monkeypatches `_signin_with_jwt` to
      ...
      `backend/tests/unit/services/data_ingestion/test_base_tableau_provider.py`.

- [x] **Step 4: Run the test**

Run: `cd backend && uv run pytest tests/unit/services/data_ingestion/test_base_tableau_provider.py -v`
Expected: PASS.
...
```bash
cd backend && make type-check && make lint
git add backend/app/services/data_ingestion/api_providers/base_tableau_api_provider.py backend/tests/unit/services/data_ingestion/test_base_tableau_provider.py
...
- [x] **Step 1: Reduce the class to its module-specific surface.** It now
...
- [x] **Step 2: Run the existing travel provider tests**
...
- [x] **Step 3: Remove now-dead env settings everywhere.** Grep first, then
      ...
      `backend/app/core/config.py` (lines 153-164), `backend/.env.example`
      (lines 95-101) and `helm/templates/backend-deployment.yaml` (lines
      196-241; the connection vars are `secretKeyRef` entries). **Keep** the
      knobs (`TABLEAU_VERIFY_SSL`, `TABLEAU_REQUEST_TIMEOUT_SECONDS`,
      `TABLEAU_REST_MIN_API_VERSION`, `TABLEAU_MAX_FIELDS`) in all three; in
      helm, `TABLEAU_VERIFY_SSL` is currently secret-sourced — move it to a
      plain value beside the other knobs.
...
- Test: `backend/tests/unit/services/data_ingestion/test_headcount_members_api_provider.py`

**Interfaces:**

- Consumes: `BaseTableauApiProvider`; produces member `DataEntry.data` dicts
  (`name`, `sius_code`, `user_institutional_id`, `fte`, `note`). There is no
  `HeadCountCreate` schema — confirm the exact keys against existing member
  data entries before finalizing `_build_data_entry`.
...
- [x] **Step 1: Write the failing transform test** (pins the mapping, not the
...
Run: `cd backend && uv run pytest tests/unit/services/data_ingestion/test_headcount_members_api_provider.py -v`
Expected: FAIL — module not found.
...
class HeadcountMembersApiProvider(BaseTableauApiProvider):
    ...
    async def _load_data(self, data: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        # via _build_data_entry and persist with the same DataEntrySourceEnum
        # member the travel provider uses (confirm the exact name in
        # app/models/data_entry.py). See professional_travel_api_provider
        # ._load_data for the call shape.
        ...
```
...
**Shipped:** the lift happened — `ingest` and its shared helpers live in
`BaseTableauApiProvider`; each subclass keeps only its own `INGEST_NOUN`,
`MISSING_UNIT_REASON` and `_success_status_message`.

- [x] **Step 4: Register in the factory** — add to `PROVIDERS` in
...
```bash
cd backend && make type-check && make lint
git add backend/app/services/data_ingestion/api_providers/headcount_members_api_provider.py backend/app/services/data_ingestion/provider_factory.py backend/tests/unit/services/data_ingestion/test_headcount_members_api_provider.py
...
- [x] **Step 1:** Add a Pinia store with typed calls mirroring
  ...
  - `testConnection(connector)` → `POST /connectors/{connector}/test`
    The repo uses `openapi-typescript` (`make gen-api-types` →
    `src/types/api/openapi.d.ts`); regenerate once the backend routes exist
    and type the store against the generated paths.

- [x] **Step 2:** `cd frontend && make type-check && npm run lint`
- [x] **Step 3:** Commit.
...
- [x] **Step 1:** Replace the API-connect section with:
...
- [x] **Step 2:** Replace `connectAndSync()` so it:
...
- [x] **Step 4:** `cd frontend && make type-check && npm run lint`
- [x] **Step 5:** Manual walkthrough: open dialog for professional_travel → pick
...
## Delivery notes (2026-07-06)

Shipped with these divergences from the letter of the plan, found and fixed in
the final whole-branch review:

- Headcount is reachable in the UI (`hasApi: true` on the member row); its
  `CAPTION_*` constants stay placeholders until read-metadata runs against the
  real LUID — a sync fails loud at caption validation until then.
- `POST …/test` is audited (connector + boolean outcome, never the secret) and
  wraps the whole probe, so an undecryptable stored secret returns a generic
  failure instead of a 500.
- `/sync/dispatch` maps the provider's "no connection/datasource configured"
  `ValueError` to a 503 carrying the operator-facing message.
- `connector_connections.updated_at` / `connector_datasources.updated_at`
  advance on update (`onupdate`), matching the `data_entry` convention.

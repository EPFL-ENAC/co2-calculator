...
- Adds `DERIVED_DATA_ENTRY_TYPES = {building: [building_embodied_energy]}`
  ...
  det-pinned building re-upload deletes stale companions along with the
...
- Runs the derivation **synchronously, inline**, at the tail of
  ...
  new chained job. Since companion rows don't need priced emissions to
...
- Fetches the just-inserted `building` rows back via a new
  ...
  (`app/repositories/data_entry_repo.py:104-134`), and the companion's
...
- Fans out a **parallel sibling** `emission_recalc(building_embodied_energy)`
  ...
  sequentially after. Safe specifically because companion rows already
...
(`backend/app/workflows/embodied_energy.py`) derives a companion
...
### 2.2 Bulk companion creation — reuse #2121's placement, not its transform

Add `EmbodiedEnergyWorkflow.create_companions_for(parent_entries:
...
`building_embodied_energy` companions:
...
- **No diff/reconcile logic needed.** #2121's delete-then-recreate strategy
  ...
  cleanup of stale companions already happened via
...
- `EmbodiedEnergyWorkflow(session).create_companions_for(parents)`:
...
- `base_csv_provider.py`, `_finalize_and_commit`: where #2121 added
  ...
  `_create_derived_companions` that — for each `derived_type` in
...
data_entry_type_id=pinned_det`) and delegates to the type-specific
  ...
  (`EmbodiedEnergyWorkflow.create_companions_for`, buildings only), so this
...
creating one room in the UI still sees the companion embodied-energy entry
...
- `backend/app/services/data_ingestion/base_csv_provider.py` — replace
  ...
  with the generic `_create_derived_companions` hook (§2.3) that calls into
...
- New unit tests for `_resolve_building_embodied_energy_data` (ref-data hit
  ...
  fallback) and `create_companions_for` (bulk-insert shape, skip-on-missing
  semantics) — mirrors
...
- Extend `test_buildings_csv_pg.py`'s existing `dispatch_csv_and_wait`
  ...
  case asserting stale companions are replaced, not duplicated (exercises
...
- Regression test for the frontend-smuggling bug: a `building` entry
  ...
  non-JS client would send) must now also get a correct companion — today

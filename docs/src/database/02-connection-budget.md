...
pooling, which our code tolerates since #2689: xact-scoped locks,
`ON COMMIT DROP` staging tables, no session state) or has a pool sized for the fleet. Until then the ceiling rule is the

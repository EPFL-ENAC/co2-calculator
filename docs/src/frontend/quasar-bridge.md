...
summary: Tracking document for Quasar CSS variables not yet bridged to our token system.
---

# Quasar Bridge: Missing Variables

Quasar ships its own SCSS variable surface; our app exposes a curated design-token layer.
The bridge file `frontend/src/css/_quasar-bridge.scss` maps our tokens to Quasar variables so
Quasar components inherit our visual identity. This page tracks which Quasar variables are
**not yet bridged** and the priority for adding them. For the bridge architecture itself
see [Design Tokens](02-design-tokens.md).

Reference: <https://quasar.dev/style/sass-scss-variables#variables-list> (Quasar v2.x).

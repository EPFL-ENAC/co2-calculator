 ...
 * answering, surfacing to users as a timeout on a healthy system. That route
 * no longer exists (#2391 decision 1 replaced it with the taxonomy lookup
 * exercised below), but the client-wide ceiling it exposed is the same one
...
test('a taxonomy lookup slower than ky default still resolves', async ({
  mount,
  page,
}) => {
  ...
  await page.route(
    '**/api/v1/taxonomies/module/equipment/scientific**',
    async (route) => {
      ...
      await route.fulfill({
        ...
        body: JSON.stringify({
          ...
          children: [
            { name: 'a', label: 'A', children: null },
            { name: 'b', label: 'B', children: null },
          ],
        }),
      });
    },
  ...
  const component = await mount(SlowFactorsHarness, {
    props: { moduleType: 'equipment', dataEntry: 'scientific', year: '2025' },
  });
  ...
  await expect(component.getByTestId('child-count')).toHaveText('2');
});

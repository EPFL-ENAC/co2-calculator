  ...
  >
    <q-card v-if="entry" class="audit-detail-drawer">
      ...
      <q-card-section class="metadata-section">
        <h6 class="section-title">{{ t('audit_detail_metadata') }}</h6>
        <div class="meta-grid">
          <div class="meta-item">
            <span class="meta-label">{{ t('audit_detail_entity_type') }}</span>
            <span class="meta-value">{{ entry.entity_type }}</span>
          </div>
          <div class="meta-item">
            <span class="meta-label">{{ t('audit_detail_entity_id') }}</span>
            <span class="meta-value">{{ entry.entity_id }}</span>
          </div>
          <div class="meta-item">
            <span class="meta-label">{{ t('audit_detail_version') }}</span>
            <span class="meta-value">{{ entry.version }}</span>
          </div>
          <div class="meta-item">
            <span class="meta-label">{{ t('audit_detail_user') }}</span>
            <span class="meta-value">
              {{
                entry.changed_by_display_name ||
                (entry.changed_by ?? t('audit_user_unknown'))
              }}
            </span>
          </div>
          <div class="meta-item">
            <span class="meta-label">{{ t('audit_detail_handler_id') }}</span>
            <span class="meta-value">{{ entry.handler_id }}</span>
          </div>
          <div class="meta-item">
            <span class="meta-label">{{ t('audit_detail_ip_address') }}</span>
            <span class="meta-value">{{ entry.ip_address }}</span>
          </div>
          <div v-if="entry.route_path" class="meta-item">
            <span class="meta-label">{{ t('audit_detail_route') }}</span>
            <span class="meta-value">{{ entry.route_path }}</span>
          </div>
          <div v-if="entry.change_reason" class="meta-item meta-item--full">
            <span class="meta-label">{{ t('audit_detail_reason') }}</span>
          ...
          >
            <span class="meta-label">{{ t('audit_detail_handled_ids') }}</span>
      ...
      <q-card-section v-if="hasDiff" class="diff-section">
        <h6 class="section-title">{{ t('audit_detail_changes') }}</h6>
        ...
        >
          ...
          >
            ...
            <div class="diff-values">
              <div class="diff-old">
                <span class="diff-label">{{ t('audit_detail_before') }}</span>
                <code>{{ formatValue(val.old) }}</code>
              </div>
              <span class="diff-arrow">→</span>
              <div class="diff-new">
                <span class="diff-label">{{ t('audit_detail_after') }}</span>
        ...
        <div
          ...
          "
        >
          <div class="diff-group-label text-positive">
            {{ t('audit_detail_added') }}
          </div>
          <div
        ...
        <div
          ...
          "
        >
          <div class="diff-group-label text-negative">
            {{ t('audit_detail_removed') }}
          </div>
          <div

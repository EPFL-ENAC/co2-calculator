...
const filteredOptionsMap = computed(() => {
  ...
  visibleFields.value.forEach((inp) => {
    ...
    const dynamicOpts = dynamicOptionsById.value[inp?.optionsId ?? ''];
    const baseOptions =
...
function getFilteredOptions(
  ...
  return opts.map((opt) => {
    ...
    if (taxoOptNode) {
      return { ...opt, label: taxoOptNode.label };
    }
    return opt;
  });
}
...
function getFieldLoading(inp: ModuleField): boolean {
  if (!inp.optionsId) return false;
  return loadingByOptionsId.value[inp.optionsId] ?? false;
}

...
interface UnitData {
  ...
  completion:
    ...
    | Record<string, ModuleCompletion>; // Year-based or flat
  completion_counts: {
    validated: number;
    in_progress: number;
    default: number;
  ...
  last_update: string;
  outlier_values: number; // Total outlier values
  expected_total?: number; // Expected total module-year combinations
}
...
const unitData = ref<UnitData | null>(null);
...
watch(
  ...
  (newData) => {
    if (newData) {
      // Wait for availableYears to be computed, then set year
      const years = availableYears.value;
      if (years.length > 0) {
        // Always set to latest year when new data loads
        selectedYearInDialog.value = years[0];
      }
    } else {
...
async function fetchUnit() {
  ...
  error.value = null;
  try {
    // Fetch without years parameter to get all year data
    const data = await api
      .get(`backoffice/unit/${props.unitId}`)
      .json<UnitData>();
...
const moduleRows = computed<ModuleRow[]>(() => {
  ...
  return MODULE_CARDS.map((card) => {
    ...
    let status = t(MODULE_STATES.Default);
    let statusColor = 'red';

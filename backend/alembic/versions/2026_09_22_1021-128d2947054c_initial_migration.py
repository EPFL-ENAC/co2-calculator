...
Revision ID: 128d2947054c
Revises:
Create Date: 2026-09-22 10:21:50.619350
...
revision: str = "128d2947054c"  # noqa: F841
...
# Hand content (#2904): labels for code-valued classification fields. The
# stored value is a code in any locale, so both languages are seeded; nothing
# else seeds these rows (formerly migrations 3b5609f893f4, fd12a7a0946f,
# 7bff78de3264). A later factor-CSV upload upserts over the fr rows.
_LABELS: dict[str, dict[str, tuple[str, str]]] = {
    "sius_code": {
        ...
        "53": ("Scientific collaborators", "Collaborateur·trices scientifiques"),
        "54": (
            "Scientific and doctoral assistants",
            "Assistant·e·s et/ou doctorant·e·s",
        ),
        "56": ("Managerial staff", "Personnel de direction de la haute école"),
...
def upgrade() -> None:
    """Upgrade schema."""
    # Hand content: the trigram GIN indexes need pg_trgm, which autogenerate
    # cannot emit (see alembic/CUSTOM_DB_OBJECTS.md).
    ...
    classification_translations = op.create_table(
    ...
    op.bulk_insert(
        classification_translations,
        [
            {"field_name": field, "value": value, "lang": lang, "label": label}
    ...
    op.create_index(
        ...
        "connector_datasources",
        ["module_type_id", sa.literal_column("COALESCE(data_entry_type_id, 0)")],
        unique=True,
    ...
    op.create_table(
        ...
        sa.Column(
            "budget_currency", sqlmodel.sql.sqltypes.AutoString(length=8), nullable=True
        ),
    ...
    op.create_index(
        "ix_factors_res_name",
        "factors",
        ["data_entry_type_id", "year", sa.literal_column("(classification->>'name')")],
        unique=False,
    ...
    op.create_table(
        ...
        sa.Column("updated_at", sa.DateTime(), nullable=False),
        sa.CheckConstraint(
            "data IS NOT NULL AND json_typeof(data) = 'object' AND data::jsonb <> '{}'::jsonb",
            name="ck_data_entries_data_is_non_empty_object",
        ),
        sa.CheckConstraint("data IS NOT NULL", name="ck_data_entries_data_not_null"),
        sa.ForeignKeyConstraint(
    ...
    op.create_index(
        ...
        unique=True,
        postgresql_where=sa.text(
            "data_entry_type_id = 1 AND data ->> 'user_institutional_id' IS NOT NULL"
        ),
    )
    op.create_table(

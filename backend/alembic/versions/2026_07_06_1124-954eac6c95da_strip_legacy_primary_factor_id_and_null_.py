...
- A purchase entry persisted with an explicit-null OR blank
  ``purchase_institutional_code`` (the pre-1661 update DTO validated
  neither) would be rejected by the new update validator on EVERY PATCH
  (present-but-null/blank is invalid; key-absent means "not updating"),
  making the row permanently un-editable. Stripping the poisoned key
...
def upgrade() -> None:
    ...
    op.execute(
        ...
        WHERE data::jsonb ? 'purchase_institutional_code'
          AND (
            data::jsonb ->> 'purchase_institutional_code' IS NULL
            OR trim(data::jsonb ->> 'purchase_institutional_code') = ''
          )
        """
    )

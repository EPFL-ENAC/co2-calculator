...
CSV_PATH = (
    Path(__file__).parent.parent.parent / "SEED_DATA" / "seed_plane_impact_factors.csv"
)
...
async def seed_plane_factors() -> None:
    ...
    async with SessionLocal() as session:
        ...
        ids = [
            f.id
            for f in existing
            if f.emission_type_id == EmissionTypeEnum.flight.value and f.id
        ]
        if ids:
        ...
        with open(CSV_PATH, encoding="utf-8") as f:
            for row in csv.DictReader(f):
                ...
                factor = await service.prepare_create(
                    emission_type_id=EmissionTypeEnum.flight.value,
                    is_conversion=False,

...
class TravelCalculationService:
    ...
    async def calculate_train_emissions(
        ...
        Geography rules:
        1. Use destination country impact factor
        2. If destination country not in table → use RoW (Rest of World)
        ...
        if dest_country:
            ...
            countrycode = "RoW"  # Fallback if destination country is None

        # 4. Get impact_score from factor table
        ...
        logger.debug(
            ...
            f"dest_country={dest_country}, used_country={countrycode}, "
            f"impact_score={impact_score}, class={class_}, "
            f"trips={number_of_trips}, kg_co2eq={kg_co2eq:.2f}"
        )

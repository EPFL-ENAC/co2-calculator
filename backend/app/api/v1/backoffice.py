...
async def list_backoffice_units(
    ...
    unit_reporting_data = []
    for item in data:
        if isinstance(item, CarbonReport):
            # suboptimal
...
#     MAX(crm.status) FILTER (WHERE crm.module_type_id = 2) AS professional_travel_status,
#     MAX(crm.status) FILTER (WHERE crm.module_type_id = 3) AS buildings_status,
#     MAX(crm.status) FILTER (WHERE crm.module_type_id = 4) AS equipment_electric_consumption_status,
#     MAX(crm.status) FILTER (WHERE crm.module_type_id = 5) AS purchase_status,
#     MAX(crm.status) FILTER (WHERE crm.module_type_id = 6) AS research_facilities_status,
#     MAX(crm.status) FILTER (WHERE crm.module_type_id = 7) AS external_cloud_and_ai_status,
...
# GROUP BY cr.id, cr.year, cr.unit_id, cr.last_updated, cr.completion_progress, cr.overall_status
# ORDER BY cr.id;
...
async def export_detailed_reporting(
    ...
    """
        ...
        We can do it faster if we skip the DTO validation and just dump the raw data from the database,
        like so

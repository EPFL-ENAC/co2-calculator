...
def resolve_flight_factor(
    ...
    Args:
        ...
        factors: Candidate flight factors (pre-filtered by kind='flight')
...
def resolve_train_factor(
    ...
    dest_country = dest.countrycode or "RoW"
    factor = next(
        (f for f in factors if f.classification.get("countrycode") == dest_country),
        None,
    )
    if not factor:
        # Fallback to RoW
        factor = next(
            (f for f in factors if f.classification.get("countrycode") == "RoW"),
            None,
        )
    return distance_km, factor

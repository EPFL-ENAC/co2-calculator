...
class UnitRepository:
    ...
    async def get_by_code(self, code: str | None) -> Optional[Unit]:
        """Get unit by code (string identifier)."""
        if not code:
            return None
        result = await self.session.exec(select(Unit).where(Unit.code == code))
        return result.one_or_none()
    ...
    async def get_by_codes(self, codes: List[str]) -> List[Unit]:
        ...
        result = await self.session.exec(select(Unit).where(col(Unit.code).in_(codes)))
        return list(result.all())
    ...
    async def list(
        ...
        if code_filter:
            query = query.where(col(Unit.code).in_(code_filter))
    ...
    async def create(
        ...
        code: str,
        name: str,
        principal_user_provider_code: Optional[int] = None,
    ...
    async def upsert(
        ...
        existing = await self.get_by_code(unit_data.code)

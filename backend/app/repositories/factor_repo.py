...
class FactorRepository:
    ...
    async def list_by_family(
        ...
        stmt = select(Factor).where(col(Factor.factor_family) == factor_family)

        if not include_expired:
            stmt = stmt.where(col(Factor.valid_to).is_(None))

        stmt = stmt.order_by(col(Factor.variant_type_id), col(Factor.version).desc())

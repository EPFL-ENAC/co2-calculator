...
# time) because asyncio.Semaphore binds to the running event loop on
# first use; module import happens before any loop is running, and
# eagerly reading get_settings() here would also pin the limit before
# tests/config have a chance to override it.
...
async def run_job(job_id: int) -> None:
    ...
    Lifecycle:
      ...
      5. Spawn heartbeat task (refreshes ``locked_at`` periodically).
         Runs as its own asyncio.Task and does NOT acquire the
         semaphore — it must keep beating for the claimed job
         regardless of how many other jobs are queued on this pod.
      6. Look up handler from registry → execute it.
      7. Pre-commit preemption check: re-read the row; if
         ``locked_by`` no longer equals our pod, roll back the
         data_session and exit without state-changing the job (the
         new owner will).
      8. Commit data_session, ``finish_job`` (atomic CAS on
         ``locked_by`` + ``state=RUNNING``) to FINISHED+SUCCESS
         (or FINISHED+ERROR on handler raise).  CAS no-op return
         means a stale-lock sweep preempted us between step 7 and
         step 8 — log+exit, the new owner will close the row.
      9. ``finish_job`` stamps ``finished_at`` via ``coalesce``
         (Plan 310-C observability), so the dashboard duration
         query (finished_at - started_at) closes cleanly.
      10. Cancel heartbeat task in ``finally``; release the semaphore
          on the way out of the ``async with`` block.

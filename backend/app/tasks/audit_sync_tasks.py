...
async def sync_pending_audit_records_task(db: AsyncSession) -> Dict[str, Any]:
    """
...
async def retry_failed_audit_records_task(
    ...
    try:
        sync_service = AuditSyncService(db)
        result = await sync_service.retry_failed_audit_records(max_retries=max_retries)
...
async def sync_audit_records_with_elasticsearch(
    ...
    """
    # Each task should get its own database session
    db1 = await get_db_session()
    db2 = await get_db_session()

    # Schedule the sync task
    background_tasks.add_task(sync_pending_audit_records_task, db1)

    # Schedule the retry task (run less frequently)
    # In production, this would be scheduled separately via a task queue
    background_tasks.add_task(retry_failed_audit_records_task, db2)

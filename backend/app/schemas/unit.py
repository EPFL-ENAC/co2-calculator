...
class UnitWithUserRole(BaseModel):
    ...
    id: str = Field(..., description="EPFL unit/department ID")
    name: str = Field(
    ...
    principal_user_id: Optional[str] = Field(None, description="Principal user ID")
...
class UnitBase(BaseModel):
    ...
    id: str = Field(..., description="EPFL unit/department numeric ID")
    name: str = Field(
        ..., min_length=1, max_length=255, description="Unit name like 'ENAC-IT4R'"
    )
    role: str = Field(..., description="User role within the unit")
    principal_user_id: str = Field(..., description="Principal user EPFL numeric ID")

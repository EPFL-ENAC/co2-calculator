...
class TestCalculateTrainEmissions:
    ...
    @pytest.mark.asyncio
    async def test_calculate_train_emissions_uses_destination_country(
        self, mock_session
    ):
        """Test that train emissions use destination country factor."""
        origin = Location(
            transport_mode="train",
            name="Zurich HB",
            latitude=47.3782,
        ...
        # Should use FR factor (0.02) not CH factor
        assert kg_co2eq > 0.0
        # Verify the query was made for FR countrycode

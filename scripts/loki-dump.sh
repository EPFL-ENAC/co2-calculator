...
epoch() { python3 -c 'import datetime as d, sys; print(int(d.datetime.fromisoformat(sys.argv[1]).replace(tzinfo=d.UTC).timestamp()))' "$1"; }
ENV="${1:-prod}"; DAY="${2:-$(python3 -c 'import datetime as d; print((d.datetime.now(d.UTC) - d.timedelta(days=1)).date())')}"; FILTER="${3:-}"
case "$ENV" in
...
CTX=$(oc config get-contexts -o name | grep -m1 "^$NS/") || { echo "no oc context for $NS: oc login there first" >&2; exit 1; }
...
start=$(epoch "$DAY"); end=$((start + 86400)); : > "$OUT"
for ((t = start; t < end; t += 300)); do
  chunk=$(curl -sSf -H "Authorization: Bearer $TOKEN" "$URL" \
